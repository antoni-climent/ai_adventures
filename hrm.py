...
def get_data_sample():
    ...
    x = torch.cat((torch.tensor(n1, dtype=torch.long),torch.tensor(n2, dtype=torch.long)))
    y = torch.tensor(s, dtype=torch.float32)
    return x, y
    ...
    def train_net(self, z_init, optimizer, epochs, N_supervision):
        for _ in range(epochs):
            ...
            for _ in range(N_supervision):
                z, y_hat = self.forward(z, x, N=20, T=5) 
...
if __name__ == "__main__":
    d_model = 16
    model = HRM(d_model=d_model, nhead=2, num_layers=2, out_size=9)
    optimizer = torch.optim.AdamW(model.parameters(), lr=0.0000000001)

    # Training 
    z = torch.empty(d_model, dtype=torch.float32)
